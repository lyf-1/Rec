    ...
    def _build_graph(self):
        ...
        for i, h_dim in enumerate(self.h_dim):
            ...
            if self.ubn:
                ...
                with tf.variable_scope('ln'+str(i), reuse=tf.AUTO_REUSE):
                    gamma = tf.get_variable('gamma', shape=[h_dim], initializer=tf.ones_initializer())
                    beta = tf.get_variable('beta', shape=[h_dim], initializer=tf.zeros_initializer())
                x = norm_x * gamma + beta
